...
parser.add_argument('--name', '-n', type=str, default='lrs2')
...
sketch_root=os.path.dirname(args.landmarks_root) + "/sketch/"
face_root=os.path.dirname(args.landmarks_root) + "/face/"
filelist_name = args.name
...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            if self.split != "train":
                for idx in T_idxs:
                    T_sketch.append(cv2.imread(os.path.join(sketch_root, vid_name, str(idx) + ".png")))
                    T_face.append(cv2.imread(os.path.join(face_root, vid_name, str(idx) + ".png")))
                T_sketch = np.stack(T_sketch)
                T_face = np.stack(T_face)
            T_sketch = torch.tensor(T_sketch)
            T_face = torch.tensor(T_face)
            ...
            #  return value
            return T_mels,    T_pose,   T_content, T_sketch, T_face,Nl_pose,Nl_content
...
def evaluate(model, val_data_loader):
    ...
    for epoch in tqdm(range(eval_epochs),total=eval_epochs):
        ...
        for step, (T_mels,T_pose,T_content,T_skecth,T_face,Nl_pose,Nl_content) in prog_bar:
            model.eval()
            ...
            # draw sketch
            if True or epoch == 0:
                T_pose = T_pose.view([-1] + list(T_pose.shape[-2:])).cpu().numpy()
                T_content = T_content.cpu().numpy()
                T_skecth = T_skecth.view([-1] + list(T_skecth.shape[2:])).cpu().numpy()
                T_face = T_face.view([-1] + list(T_face.shape[2:])).cpu().numpy()
                predict_content = predict_content.cpu().numpy()
                for i in range(T_content.shape[0]):
                    gt_sketch = draw_pose_and_content(T_pose[i], T_content[i])
                    predict_sketch = draw_pose_and_content(T_pose[i], predict_content[i])

                    output_sketch = np.concatenate([T_face[i], T_skecth[i], gt_sketch, predict_sketch], axis=1)
                    output_path = os.path.join("evaluation_visualized/{}/h_{}_{}.jpg".format(
                        global_step,
                        epoch,
                        i,
                    ))
                    os.makedirs(os.path.dirname(output_path), exist_ok=True)
                    cv2.imwrite(output_path, output_sketch)

                    output_sketch = np.concatenate([T_face[i], T_skecth[i], gt_sketch, predict_sketch], axis=0)
                    output_path = os.path.join("evaluation_visualized/{}/v_{}_{}.jpg".format(
                        global_step,
                        epoch,
                        i,
                    ))
...
if __name__ == '__main__':
    ...
    while global_epoch < 9999999999:
        ...
        for step, (T_mels, T_pose, T_content, T_sketch, T_face, Nl_pose, Nl_content) in prog_bar:
