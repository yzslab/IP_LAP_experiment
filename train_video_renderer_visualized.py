...
def save_sample_images_gen(T_frame_sketch, ref_N_frame_img, wrapped_ref, generated_img, gt, eval_epoch, global_step, checkpoint_dir):
    ...
    for batch_idx, c in enumerate(collage):   # require (B,T,H,W,3)
        for t in range(len(c)):
            cv2.imwrite('{}/{}_{}_{}.png'.format(folder, eval_epoch, batch_idx, t), c[t])
...
def evaluate(model, val_data_loader):
    ...
    for epoch in range(eval_epochs):
        ...
        for step, (T_frame_img, T_frame_sketch, ref_N_frame_img, ref_N_frame_sketch,T_mels) in prog_bar:
            ...
            ssims.append(ssim)
            fids.append(fid)
        save_sample_images_gen(T_frame_sketch, ref_N_frame_img, wrapped_ref,generated_img, gt, epoch, global_step, checkpoint_dir)
        #                         (B,T,3,H,W)  (B,ref_N,3,H,W)  (B*T,3,H,W) (B*T,3,H,W)(B*T,3,H,W)
...
if __name__ == '__main__':
    ...
    if finetune_path is not None:  ###fine tune
        load_checkpoint(finetune_path, model, optimizer, reset_optimizer=False, overwrite_global_states=True)
